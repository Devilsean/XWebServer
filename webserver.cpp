...
void WebServer::eventListen() {
  ...
  assert(ret >= 0);
  ret = listen(m_listenfd, 5); // 监听5个连接
  assert(ret >= 0);
  ...
  ret = socketpair(PF_UNIX, SOCK_STREAM, 0, m_pipefd); // 创建管道
  assert(ret != -1);
...
bool WebServer::dealwithsignal(bool &timeout, bool &stop_server) {
  ...
  int sig;
  char signals[1024];
  ret = recv(m_pipefd[0], signals, sizeof(signals), 0); // 读取管道
  if (ret == -1) {
    return false;
  } else if (ret == 0) {
    return false;
