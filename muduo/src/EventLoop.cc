...
int createEventfd() {
  ...
  if (evtfd < 0) {
    MUDUO_LOG_FATAL("eventfd error:%d\n", errno);
  }
  return evtfd;
}
...
EventLoop::EventLoop()
    : looping_(false), quit_(false), threadId_(CurrentThread::tid()),
      pollRetureTime_(0), poller_(Poller::newDefaultPoller(this)),
      wakeupFd_(createEventfd()), wakeupChannel_(new Channel(this, wakeupFd_)),
      callingPendingFunctors_(false) {
  MUDUO_LOG_DEBUG("EventLoop created %p in thread %d\n", this, threadId_);
  if (t_loopInThisThread) {
    MUDUO_LOG_FATAL("Another EventLoop %p exists in this thread %d\n",
...
void EventLoop::loop() {
  ...
  MUDUO_LOG_INFO("EventLoop %p start looping\n", this);
  ...
  MUDUO_LOG_INFO("EventLoop %p stop looping.\n", this);
  looping_ = false;
}
...
void EventLoop::handleRead() {
  ...
  if (n != sizeof(one)) {
    MUDUO_LOG_ERROR("EventLoop::handleRead() reads %lu bytes instead of 8\n",
                    n);
  }
}
...
void EventLoop::wakeup() {
  ...
  if (n != sizeof(one)) {
    MUDUO_LOG_ERROR("EventLoop::wakeup() writes %lu bytes instead of 8\n", n);
  }
}
