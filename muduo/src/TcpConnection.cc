...
static EventLoop *CheckLoopNotNull(EventLoop *loop) {
  if (loop == nullptr) {
    MUDUO_LOG_FATAL("%s:%s:%d mainLoop is null!\n", __FILE__, __FUNCTION__,
                    __LINE__);
  }
  return loop;
}
      ...
      peerAddr_(peerAddr), highWaterMark_(64 * 1024 * 1024) // 64M
{
  ...
  MUDUO_LOG_INFO("TcpConnection::ctor[%s] at fd=%d\n", name_.c_str(), sockfd);
  socket_->setKeepAlive(true);
}

TcpConnection::~TcpConnection() {
  MUDUO_LOG_INFO("TcpConnection::dtor[%s] at fd=%d state=%d\n", name_.c_str(),
                 channel_->fd(), (int)state_);
}

void TcpConnection::send(const std::string &buf) {
  if (state_ == kConnected) {
    if (loop_->isInLoopThread()) // 这种是对于单个reactor的情况
...
void TcpConnection::sendInLoop(const void *data, size_t len) {
  ...
  {
    MUDUO_LOG_ERROR("disconnected, give up writing");
  }
    ...
    } else // nwrote < 0
    {
      ...
      if (errno !=
          EWOULDBLOCK) // EWOULDBLOCK表示非阻塞情况下没有数据后的正常返回
                       // 等同于EAGAIN
      {
        MUDUO_LOG_ERROR("TcpConnection::sendInLoop");
...
void TcpConnection::handleRead(Timestamp receiveTime) {
  ...
  {
    errno = savedErrno;
    MUDUO_LOG_ERROR("TcpConnection::handleRead");
    handleError();
  }
}
...
void TcpConnection::handleWrite() {
  if (channel_->isWriting()) {
    ...
    // A. 优先处理普通的 outputBuffer_ 数据（原有逻辑）
    ...
    // B. 如果 Buffer 发完了，接着处理没发完的文件（新增逻辑）
    if (outputBuffer_.readableBytes() == 0 && fileRemaining_ > 0) {
      ssize_t n =
          ::sendfile(socket_->fd(), fileFd_, &fileOffset_, fileRemaining_);
      if (n >= 0) {
        fileRemaining_ -= n;
      } else {
        if (errno != EWOULDBLOCK && errno != EAGAIN) {
          MUDUO_LOG_ERROR(
              "TcpConnection::handleWrite sendfile failed, errno=%d, reason=%s",
...
void TcpConnection::handleClose() {
  MUDUO_LOG_INFO("TcpConnection::handleClose fd=%d state=%d\n", channel_->fd(),
                 (int)state_);
  setState(kDisconnected);
  channel_->disableAll();
...
void TcpConnection::handleError() {
  ...
  MUDUO_LOG_ERROR("TcpConnection::handleError name:%s - SO_ERROR:%d\n",
                  name_.c_str(), err);
}
...
void TcpConnection::sendFile(int fileDescriptor, off_t offset, size_t count) {
  if (connected()) {
    ...
    MUDUO_LOG_ERROR("TcpConnection::sendFile - not connected");
  }
}
...
void TcpConnection::sendFileInLoop(int fileDescriptor, off_t offset,
                                   size_t count) {
  if (state_ == kDisconnecting) {
    ::close(fileDescriptor); // 连接断了，任务取消，记得关文件
    return;
  }
  ...
  if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0) {
    n = ::sendfile(socket_->fd(), fileDescriptor, &offset, remaining);
    if (n >= 0) {
      remaining -= n;
      // 发完了必须关文件
      if (remaining == 0) {
      ...
      if (errno != EWOULDBLOCK && errno != EAGAIN) {
        MUDUO_LOG_ERROR("TcpConnection::sendFileInLoop - sendfile error");
        ::close(fileDescriptor); // 出错了也要关，防止泄露
        return;
      }
    }
  }

  // 保存状态以便 handleWrite 接管
  if (remaining > 0) {
    fileFd_ = fileDescriptor;
    fileOffset_ = offset;
