...
static int createNonblocking() {
  int sockfd = ::socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK | SOCK_CLOEXEC,
                        IPPROTO_TCP);
  if (sockfd < 0) {
    MUDUO_LOG_FATAL("%s:%s:%d listen socket create err:%d\n", __FILE__, __FUNCTION__,
              __LINE__, errno);
  }
  return sockfd;
}
...
Acceptor::~Acceptor() {
  ...
  acceptChannel_
      .remove(); // 调用EventLoop->removeChannel => Poller->removeChannel
...
void Acceptor::handleRead() {
  ...
  if (connfd >= 0) {
    ...
    MUDUO_LOG_ERROR("%s:%s:%d accept err:%d\n", __FILE__, __FUNCTION__, __LINE__,
              errno);
    if (errno == EMFILE) {
      MUDUO_LOG_ERROR("%s:%s:%d sockfd reached limit\n", __FILE__, __FUNCTION__,
                __LINE__);
    }
  }
}