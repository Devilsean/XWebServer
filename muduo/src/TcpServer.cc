...
TcpServer::TcpServer(EventLoop *loop, const InetAddress &listenAddr,
    ...
    : loop_(CheckLoopNotNull(loop)), ipPort_(listenAddr.toIpPort()),
      ...
      messageCallback_(), started_(0), nextConnId_(1) {
...
TcpServer::~TcpServer() {
  for (auto &item : connections_) {
    ...
    item.second.reset(); // 把原始的智能指针复位 让栈空间的TcpConnectionPtr
                         // conn指向该对象 当conn出了其作用域
                         // 即可释放智能指针指向的对象
    // 销毁连接
...
void TcpServer::newConnection(int sockfd, const InetAddress &peerAddr) {
  ...
  ++nextConnId_; // 这里没有设置为原子类是因为其只在mainloop中执行
                 // 不涉及线程安全问题
  ...
  if (::getsockname(sockfd, (sockaddr *)&local, &addrlen) < 0) {
    MUDUO_LOG_ERROR("sockets::getLocalAddr");
  }
...
void TcpServer::removeConnectionInLoop(const TcpConnectionPtr &conn) {
  MUDUO_LOG_INFO("TcpServer::removeConnectionInLoop [%s] - connection %s\n",
