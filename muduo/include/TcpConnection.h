...
private:
  ...
  // 文件发送状态（用于sendfile续传）
  int sendFileFd_;           // 正在发送的文件描述符
  off_t sendFileOffset_;     // 文件当前偏移量
  size_t sendFileRemaining_; // 剩余待发送字节数

  // 数据缓冲区
  Buffer inputBuffer_;  // 接收数据的缓冲区
  Buffer outputBuffer_; // 发送数据的缓冲区 用户send向outputBuffer_发

  int fileFd_;           // 当前发送的文件描述符
  off_t fileOffset_;     // 当前发送的偏移量
  size_t fileRemaining_; // 剩余待发送字节数
};
